<?php
	class User {
        ...
        {
            $statement = $this->connection->prepare("INSERT INTO contact (message, id) VALUES (?, ?)");
            // Bind all values to the placeholders
        ...
        public function getAllVisibleQueries($limit, $offset) {
            try {
                $statement = $this->connection->prepare("SELECT * FROM queries WHERE visibility = :visibility ORDER BY created_at DESC LIMIT :limit OFFSET :offset");
                ...
                error_log('Error retrieving queries: ' . $e->getMessage());
                return false;
            }
        }
