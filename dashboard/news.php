                                ...
                                <tbody>
                                    <?php foreach (!empty($user->getAllNews()) ? $user->getAllNews() : [] as $ns): ?>
                                        <tr>
                                            <td>
                                            <?php
                                        if (isset($ns->expertId)) {
                                            $expertInfo = $user->getExpertInfo($ns->expertId);
                                            if ($expertInfo && is_object($expertInfo) && isset($expertInfo->name)) {
                                                echo $expertInfo->name;
                                            } else {
                                                $userInfo = $user->getUser($ns->expertId);
                                                if ($userInfo && is_object($userInfo) && isset($userInfo->name)) {
                                                    echo $userInfo->name;
                                                }
                                            }
                                        }
